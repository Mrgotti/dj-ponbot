...
class Thread {
  ...
  async replyToUser(
    ...
    if (isAnonymous) {
      ...
      logModUsername = `(Anonymous) (${moderator.user.username}) ${
        mainRole ? mainRole.name : "Moderator"
      }`;
    } else {
      const name = config.useNicknames
        ? moderator.nick || moderator.user.username
    ...
    if (replyAttachments.length > 0) {
      for (const attachment of replyAttachments) {
        ...
        const url = await attachments.getUrl(
          attachment.id,
          attachment.filename
        );
  ...
  async receiveUserReply(msg) {
    ...
    let threadContent = `**${msg.author.username}#${
    ...
    if (this.alert_id) {
      ...
      await this.postSystemMessage(
        `<@!${this.alert_id}> New message from ${this.user_name}`
      );
    }
  }
  ...
  async postToThreadChannel(...args) {
    try {
      ...
      if (e.code === 10003) {
        console.log(
          `[INFO] Failed to send message to thread channel for ${
            this.user_name
          } because the channel no longer exists. Auto-closing the thread.`
        );
        this.close(true);
      } else {
        throw e;
